...
class DenseIndexedMap:
    ...
    def _mark_removed_vec_id(self, removed_vec_id: torch.Tensor):
        ...
        """
        # with isin
        #self.mesh_cache.updated_vec_id = self.mesh_cache.updated_vec_id[torch.isin(self.mesh_cache.updated_vec_id, removed_vec_id, inverse=True)]
        ...
        '''
        for i in range(removed_vec_id.shape[0]):
          if removed_vec_id[i] != -1:
            self.mesh_cache.updated_vec_id = self.mesh_cache.updated_vec_id[self.mesh_cache.updated_vec_id!=removed_vec_id[i]]
        '''
    ...
    def integrate_keyframe(self, surface_xyz: torch.Tensor, surface_normal: torch.Tensor, do_optimize: bool = False, async_optimize: bool = False):
        ...
        def get_pruned_surface(enabled=True, lin_pos=None, bias=None):
            ...
            if enabled:
                ...
                return surface_xyz_normalized[focus_mask]+bias, surface_normal[focus_mask]
            else:
                return surface_xyz_normalized+bias, surface_normal
        ...
        if encoder_voxel_pos.size(0) > 0:
          ...
          bias = torch.cat([torch.tensor([[0,0,0]]),torch.eye(3)*delta],axis=0).to(surface_xyz_normalized) # 4,3
          for _ in range(4):
            idx = 3-_
            pruned_surface_xyz_normalized, pruned_surface_normal = get_pruned_surface(
                    enabled=True, lin_pos=encoder_voxel_pos, bias = bias[(idx,),:])
            ...
            # encoder_latent, J = point_encoder_w_J(self.model.encoder, gathered_surface_xyzn, self.latent_dim*3*2)

            if idx == 0: # zero
                encoder_latent_sum = net_util.groupby_reduce(pinds, encoder_latent.contiguous(), op="sum")   # (C, L) which is latent*count
                encoder_latent = net_util.groupby_reduce(pinds, encoder_latent.contiguous(), op="mean")   # (C, L) which is latent*count
                # store J
                #self.latent_Js[surface_blatent_mapping] = net_util.groupby_reduce(pinds, J.reshape((J.shape[0],-1)),op='mean') # (C,L)
                n,c = encoder_latent_sum.shape
                encoder_biased = self.latent_Js[surface_blatent_mapping].reshape((n,3,c))
                Js_m = encoder_biased - encoder_latent.unsqueeze(1)
                self.latent_Js[surface_blatent_mapping] = Js_m.transpose(1,2).reshape((n,-1))
                ...
                encoder_latent_mean = net_util.groupby_reduce(pinds, encoder_latent.contiguous(), op="mean")   # (C, L) which is latent*count
                c = encoder_latent_mean.shape[1]
                self.latent_Js[surface_blatent_mapping,c*(3-idx):c*(3-idx+1)] = encoder_latent_mean
    ...
    def remove_local(self, local_interm):
        ...
        print('remove stage 1:', time.time()-st)
        ...
        print('remove stage 2:', time.time()-st)

        self._mark_removed_vec_id(self.indexer[lp])
        print('remove stage 3:', time.time()-st)
    ...
    def fuse_local(self, local_interm):
        ...
        valid_mask = (lineared_pos>=0) * (lineared_pos < self.indexer.shape[0])
        lineared_pos = lineared_pos[valid_mask]
        ...
        def do_meshing(voxel_resolution):
            ...
            with torch.cuda.stream(self.meshing_stream):
                ...
                with torch.no_grad():
                    ...
                    if low_memory:
                        low_sdfs, low_stds = [], []
                        for i in range(0,B,64): # 64 is batchsize
                            if min(i+64,B) - i <= 0:
                                break
