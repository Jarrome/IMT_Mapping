...
class SDFTracker:
    ...
    def compute_sdf_Hg(self, n_iter: int, last_pose: Isometry, cur_delta_pose: Isometry, obs_xyz: torch.Tensor, no_grad: bool = False, debug = False):
        """
            ...
            for i_iter in list(range(group_iter_config["n"])) + [-1]:
                ...
                if i_iter != -1:
                    ...
                    try:
                        ...
                        sdf_H, sdf_g, sdf_energy = self.compute_sdf_Hg(i_iter, last_pose, cur_delta_pose, obs_xyz, i_iter == -1 , debug=True)
                        pdb.set_trace()
                        print(xi)
