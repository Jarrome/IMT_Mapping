...
def load_model(training_hyper_path: str, use_epoch: int = -1):
    ...
    model.decoder = net_module.Model(args.code_length*3*2, **args.network_specs).cuda()
        ...
        if isinstance(mdl, torch.nn.Linear):
            for k, hook in mdl._forward_pre_hooks.items():
                if isinstance(hook, WeightNorm):
                    fix_name = hook.name
        if fix_name is not None and fix_name in mdl.__dict__.keys(): # Fixed, if multiple pass decoder, weight has already been removed 
